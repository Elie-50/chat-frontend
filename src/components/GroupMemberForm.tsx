...
function GroupMemberForm({ id }: Props) {
	const { findGroupData, selectedGroup, error, addMemberToGroup } =
		useGroupStore();
	...
	const tab = (searchParams.get('tab') ?? 'members') as 'members' | 'friends';
	...
	const lastFriendRef = useCallback(
		...
		[friendsLoading, friendsResult, page, size, tab, setSearchParams],
	);

	return (
		<div className="w-full mt-10">
			<h2 className="text-lg text-center font-semibold mb-4">Group Members</h2>
			...
			{tab === 'friends' && friendsResult && (
				<section className="space-y-2">
					{friendsResult.friends.map((friend, index) => {
						...
						const isLast = index === friendsResult.friends.length - 1;
