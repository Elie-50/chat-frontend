...
const ChatBubble = ({
	...
	const uniqueColor = useMemo(
		() => stringToColor(message.sender),
		[message.sender],
	);
	const replyColor = useMemo(
		() => stringToColor(message.reply?.sender.username || ""),
		...
		<div
			className={cn(
				reply?._id === message._id &&
					"w-full bg-blue-400 rounded-sm opacity-75",
			)}
		>
			<div
				data-id={message._id}
				...
				>
					...
					<div className="flex flex-wrap items-end gap-x-2">
						{isDeleted ? (
							...
							<div
								className={`text-base leading-relaxed wrap-break-word max-w-full ${message.reply && "w-full"}`}
							>
								{/* Reply */}
								{message.reply && (
									<div
										...
										onClick={() =>
											scrollMessageIntoView(message.reply?._id as string)
										}
									>
										...
										<p className="mt-1 truncate max-w-xs">
